...
| `firmware/circuitpython/lib/` | CircuitPython libraries (CP 7.x `.mpy` format, from bundle `20230718`) |
| `firmware/rust/` | Rust+Embassy firmware (future, experimental) |
...
### New Code Guidelines
- CircuitPython code goes in `firmware/circuitpython/` (never in `original_helmut/`)
- Rust code goes in `firmware/rust/`
...
- `firmware/circuitpython/VERSION` is gitignored (generated artifact)
...
- **Release workflow** (`.github/workflows/release.yml`): Triggered by version tags
  - Packages `firmware/circuitpython/` into a zip (excludes `experiments/`, `__pycache__/`)
...
**CI enforces this** via the "CircuitPython 7.x compatibility guard" step in `ci.yml`. It greps `firmware/circuitpython/` for banned patterns and fails the build.
...
- Experiments in `firmware/circuitpython/experiments/` for isolated testing
...
Both distribution paths must include the same set of files and write the `VERSION` file. If you add a new directory under `firmware/circuitpython/`, you must add it to **both** of these:
1. `tools/deploy.sh` — dev deploy via rsync (also writes `VERSION` to device and local `firmware/circuitpython/`)
...
- New code in `firmware/circuitpython/` is owned by Maximilian Cascone
...
**Available fonts** (`firmware/circuitpython/fonts/`):
| File | Size | Used as |
|------|------|---------|
...
`firmware/circuitpython/core/config.py` handles config parsing. Key points:
...
| Path | Purpose |
|------|---------|
| `firmware/circuitpython/code.py` | **Active**: Unified firmware with config, display, bidirectional MIDI |
| `firmware/circuitpython/boot.py` | Disables autoreload; USB drive gated by `dev_mode` config flag or Switch 1 hold; applies custom drive label |
| `firmware/circuitpython/config.json` | STD10 default config (button labels, CC numbers, colors, drive name, dev_mode) |
| `firmware/circuitpython/config-mini6.json` | Mini6 template config (copy to device as config.json) |
| `firmware/circuitpython/VERSION` | Firmware version (generated, gitignored) |
| `firmware/circuitpython/core/config.py` | Config loading; `get_usb_drive_name()`, `validate_usb_drive_name()`, `get_dev_mode()`, `get_display_config()`; `STATE_OVERRIDE_FIELDS` |
| `firmware/circuitpython/core/button.py` | `ButtonState` class: toggle/momentary mode, keytimes cycling |
| `firmware/circuitpython/core/colors.py` | Color palette and `get_off_color()` utilities |
| `firmware/circuitpython/devices/std10.py` | STD10 hardware constants |
| `firmware/circuitpython/devices/mini6.py` | Mini6 hardware constants |
| `firmware/circuitpython/original_helmut/code.py` | Helmut's original firmware (reference only, DO NOT MODIFY) |
| `firmware/rust/` | Rust+Embassy firmware (future, see `firmware/rust/README.md`) |
