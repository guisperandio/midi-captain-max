<script lang="ts">
  ...
  let viewBox = $derived(deviceType === 'mini6' ? '0 0 560 560' : '0 0 840 600');
  let maxWidth = $derived(deviceType === 'mini6' ? 560 : 840);
  ...
  const LED_RING_RADIUS = 56; // Radius for LED ring around button
  const LED_RING_WIDTH = 3; // Thickness of LED ring
  ...
  function getButtonPosition(index: number): { x: number; y: number } {
    ...
    const startX = deviceType === 'mini6' ? 60 : 70;
    const startY = HEADER_HEIGHT + 30;
    return {
      x: startX + col * (BUTTON_SIZE + BUTTON_SPACING),
      y: startY + row * (BUTTON_SIZE + BUTTON_SPACING + 30)
    };
  }
...
<div class="device-layout-container">
  <svg {viewBox} class="device-svg" style="max-width: {maxWidth}px;">
    ...
    <!-- Port Labels removed for cleaner look -->
    ...
    <!-- Encoder and Expression Indicators (between rows) -->
    {#if deviceType === 'std10'}
      {@const midY = HEADER_HEIGHT + 30 + BUTTON_SIZE + 15}
      {@const encoderX = maxWidth / 2 - 40}
      {@const exp1X = maxWidth / 2 + 30}
      
      <!-- Encoder -->
      <g class="control-indicator">
        <circle cx={encoderX} cy={midY} r="16" class="encoder-circle" />
        <circle cx={encoderX} cy={midY} r="10" class="encoder-inner" />
        <line x1={encoderX} y1={midY - 10} x2={encoderX} y2={midY - 7} class="encoder-marker" stroke="#8b5cf6" stroke-width="2" />
        <text x={encoderX} y={midY + 28} class="control-label" text-anchor="middle">ENCODER</text>
      </g>
      
      <!-- Expression Pedals -->
      <g class="control-indicator">
        <rect x={exp1X - 12} y={midY - 15} width="24" height="30" rx="2" class="exp-rect" />
        <line x1={exp1X - 8} y1={midY + 6} x2={exp1X + 8} y2={midY - 6} class="exp-line" stroke="#10b981" stroke-width="2" />
        <text x={exp1X} y={midY + 28} class="control-label" text-anchor="middle">EXP 1/2</text>
      </g>
    {:else}
