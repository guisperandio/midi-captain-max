...
class ButtonPressHandler:
    ...
    def check_long_press(self, now, btn_state):
        ...
        effective_long_press = self.callbacks["get_action_cfg"](self.config, "long_press", current_keytime)

        # Determine threshold (ms) from effective config, button config, or default
        threshold_ms = self.config.get("long_press_threshold_ms", 700)
        if effective_long_press:
            if isinstance(effective_long_press, dict):
